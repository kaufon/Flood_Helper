...
export default function Request({ data }) {
  function btnCloseClick() {
    if (!confirm("Tem certeza que quer fechar o pedido?")) return;

    closeRequest(data.id)
      .then((result) => {
        alert(
          "Pedido fechado com sucesso,em alguns minutos ele deixara de existir",
        );
  ...
  function btnHelpClick() {
    ...
    donate(data.id, donationInBnb)
      .then((result) => {
        alert("Doação efetuado com sucesso,obrigado!");
        window.location.reload();
      })
      .catch((err) => {
        console.log(err);
          ...
          <div className="w-100">
            <div className="row">
              <div className="col-10">
                <h6 className="mb-0">
                  {data.title} &rsaquo;&rsaquo; {data.contact}
                </h6>
              </div>
              <div className="col-2">
            ...
            <div className="row">
              <div className="col">
                <span className="me-1 opacity-75">Metah:</span>
